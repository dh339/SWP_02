...
float distance = _DIST_MAX; //위치와 변수 초기화를 시킴 (수정 2개)
...
void loop() { 
  ...
  if ((distance == 0.0) || (distance < _DIST_MIN) || (distance > _DIST_MAX)) {
    analogWrite(PIN_LED, 255);  // 범위 밖이면 LED 꺼짐
  } 
  else {
    
    if (distance <= 200.0) {
      // 100mm에서 200mm까지는 점점 밝아짐 (active low)
      brightness = (int)(255 - (distance - 100.0) / 100.0 * 255);
    } else {
      // 200mm에서 300mm까지는 점점 어두워짐
      brightness = (int)((distance - 200.0) / 100.0 * 255);
    }
  ...
  distance = USS_measure(PIN_TRIG, PIN_ECHO); // read distance 위치 변경(수정)

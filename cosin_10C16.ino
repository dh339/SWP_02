/*
 * 도전과제1: 주차장 차단기 (Cosine Easing)
 * - 초음파로 차량 접근 감지 -> 서보 차단기 부드럽게 상승/하강
 * - 코사인 이징: ease = (1 - cos(pi*u)) / 2
...
float measureDistanceMM() {
  ...
  return 0.173f * (float)duration; // = (SND_VEL*1000*duration*1e-6)/2
}
...
// Cosine ease-in-out: u∈[0,1] -> [0,1]
float easeCosine(float u) {
  // Arduino는 PI 상수가 제공됨
  return 0.5f * (1.0f - (float)cos(PI * u));
}

void startMove(int newTarget) {
  startAngle  = gate.read(); // 직전 각도
